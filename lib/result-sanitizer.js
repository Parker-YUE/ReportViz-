...
function normalizeReportMode(value) {
  ...
  const error = new Error('报告版本无效');
  error.code = 'INVALID_REPORT_MODE';
  error.status = 400;
  throw error;
}
...
function assertScoredResult(data, reportMode) {
  ...
  const radar = Array.isArray(data?.sections)
    ? data.sections.find(section => section?.type === 'radar_score')
    : null;
  const valid = radar
    && Number.isFinite(Number(radar.total_score))
    && Array.isArray(radar.dimensions)
    && radar.dimensions.length === 5
    && radar.dimensions.every(dimension => (
  ...
  if (!valid) {
    const error = new Error('五维评分结构不完整');
    error.code = 'INVALID_SCORED_RESULT';
    error.status = 502;
    throw error;
  }
...
function sanitizeSection(section, options) {
  ...
  if (next.type === 'radar_score') {
    ...
    const preserveSourceDimensions = options.reportMode === 'scored'
      && hasExplicitSourceDimensions(next.dimensions, options.inputText);

    // AI转型报告：强制使用固定五大维度
    if (options.isAiTransform && !preserveSourceDimensions) {
...
function hasExplicitSourceDimensions(dimensions, inputText) {
  if (!Array.isArray(dimensions) || dimensions.length !== 5 || !inputText) return false;
  return dimensions.some(dimension => {
    const name = String(dimension?.name || '').trim();
    return name && String(inputText).includes(name);
  });
}
